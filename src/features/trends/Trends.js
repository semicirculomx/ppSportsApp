...
export default (props) => {
    ...
    if (!trends || !trends.length)
        return <div className="message">No hay tendencias ahora</div>
    return (
        <ListGroup variant="flush">
            {(trends.slice(0, props.length).map((itm, index )=> {
                return (
                    <ListGroup.Item
                        as={Link}
                        action
                    ...
                    >
                        {/* <small className="text-muted">En tendencia {location.name}</small> */}
                        <p className="mb-1 text-dark font-weight-bold text-capitalize">{index +1} {itm.name}</p>
                        {/* <em className="">{itm.tweet_volume + ' Picks'} </em> */}
                    </ListGroup.Item>
                )
            }))}
        </ListGroup>
